...
from eval.metrics import (answer_correct, citation_accuracy, mean_reciprocal_rank,
    precision_recall_f1, report_entry_id, retrieval_recall_at_k)
...
def evaluate_retrieval(dataset: dict) -> None:
    ...
    for case in dataset.get("qa_cases", []):
        ...
        results.append({"question": case["question"],
            "recall_at_k": retrieval_recall_at_k(result_ids, expected_ids, settings.retrieval.top_k),
            "mrr": mean_reciprocal_rank(result_ids, expected_ids)})
    if not results:
        logger.warning("retrieval_evaluation_not_run", extra={
            "reason": "qa_cases have no reviewed expected_retrieval_ids",
        })
        return
    logger.info("retrieval_scorecard", extra={"cases": len(results),
        "mean_recall_at_k": sum(item["recall_at_k"] for item in results) / len(results),
        "mean_mrr": sum(item["mrr"] for item in results) / len(results), "results": results})
...
def evaluate_generation(dataset: dict) -> None:
    ...
    for case in cases:
        ...
        if "could not complete the request" in answer.text:
            logger.warning("generation_evaluation_not_run", extra={
        ...
        results.append({"question": case["question"],
            ...
            "citation_accuracy": citation_accuracy(answer.citations, case.get("expected_citations", []))})
    logger.info("chat_scorecard", extra={"cases": len(results),
...
def main(write_candidates: bool) -> None:
    ...
    try:
        ...
        logger.exception("generation_evaluation_failed", extra={"reason": "a configured LLM provider is required"})


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Evaluate generated deltas and grounded chat.")
    parser.add_argument("--write-candidates", action="store_true", help="write candidate IDs for human review")
