...
def ocr_lines(data: dict[str, list[Any]], scale: float) -> list[dict[str, float | str]]:
    ...
    groups: dict[tuple[int, int, int], list[tuple[str, float, float, float, float, float]]] = defaultdict(list)
    for index, raw_text in enumerate(data["text"]):
        ...
        left, top, width, height = (float(data[name][index]) / scale for name in ("left", "top", "width", "height"))
