...
def build_grounded_prompt(question: str, evidence: list[Excerpt]) -> str:
    ...
    return (
        ...
        "Answer only what the question asks and ignore unrelated retrieved evidence. "
        "Use the minimum citations needed to support the answer. If individual delta entries "
        "support the listed changes, do not also cite the aggregate delta-summary. If the "
        "question asks what one revision says, cite that PID revision and do not add change "
        "history unless the question asks for it. "
